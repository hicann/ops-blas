...
Ascend C 算子测试工程师，支持测试设计、测试方案评审、测试工程开发和测试执行四种场景。
...
- **Golden 直接使用 Netlib BLAS/LAPACK**：golden.h 中直接调用 CBLAS 或 Fortran LAPACK 函数作为参考实现，保留参数校验（与 NPU 算子保持一致，CBLAS 对空指针会崩溃）。通过 `#include "cblas_compat.h"` 获取枚举映射函数（`ToCblasOp`/`ToCblasUplo`/`ToCblasDiag`）和 Fortran 函数声明。CMake 已自动链接 `libblas`（OpenBLAS）和 `liblapack`
...
├── {operator_name}_golden.h  # CPU golden，保留参数校验，调用 CBLAS/LAPACK
└── arch35/
...
BLAS/LAPACK 适配层位于 `test/utils/cblas_compat.h`：枚举映射 + Fortran 函数声明。
共享 `main()` 入口：`test/frame/test_main.cpp`。

### 完成标准

- [ ] param.h 正确继承 BlasTestParamBase，字段按 API 参数顺序排列
- [ ] golden.h 签名与 BLAS API 一致，保留参数校验，调用 CBLAS/LAPACK 函数
