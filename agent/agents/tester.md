...
├── {routine}_test.cpp            # GTest TEST_P 入口 + verify
├── {routine}_golden.h            # CPU 参考实现（enumerate + golden）
├── {routine}_testcases.csv       # CSV 参数化用例表
└── README.md                     # 测试说明
```

公共工具位于 `test/utils/`：`test_case_config.h`、`config_loader.h`、`verifier.h`、`verify_helpers.h`、`gtest_utils.h`。

### 完成标准

- [ ] CSV 用例表覆盖测试设计文档中的所有场景
- [ ] golden.h CPU 参考实现正确
- [ ] GTest TEST_P 入口完整（含异常用例、n=0、特殊值分支）
- [ ] `CMakeLists.txt` 使用 `ops_blas_add_gtest_tests`
- [ ] 编译通过：`bash build.sh --ops={operator_name}`
- [ ] ST 通过：`bash build.sh --ops={operator_name} --soc=ascend950 --run`
