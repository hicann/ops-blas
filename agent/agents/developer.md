...
4. **git 分支** — `git checkout -b blas/{operator_name}` 创建开发分支
...
ops-blas 算子按 routine 和架构组织：
```
blas/{operator_name}/
└── {routine}/
    └── archXX/
        ├── {routine}_host.cpp         # Host 侧：参数校验、Tiling、直调 Kernel
        ├── {routine}_kernel.cpp       # Kernel 侧：AscendC 类 + Kernel 入口
        └── {routine}_tiling_data.h    # TilingData 结构体（Host/Kernel 共用）
```
...
2. **代码实现**：
   - 创建 `{routine}_tiling_data.h`：定义 TilingData 结构体
   - 创建 `{routine}_kernel.cpp`：实现 AscendC Kernel 类 + `__aicore__` 入口，通过 `GM_ADDR` 接收 Tiling 指针并解析
   - 创建 `{routine}_host.cpp`：参数校验、计算 TilingData、`aclrtMalloc`+`aclrtMemcpy` 传递到 Device、`<<<>>>` 直调 Kernel
   - 架构特定代码放在 `archXX/` 子目录
