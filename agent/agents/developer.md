...
**命名规范**：
- `{family}`：snake_case 格式（如 `swap`、`geqrf_batched`、`getrf_batched`）
- `{operator_name}`：snake_case 格式（如 `sswap`、`sgeqrf_batched`、`sgetrf_batched`），**不是** API 名（`aclblasSgeqrfBatched`）
...
2. **代码实现**：
   ...
   - 创建 `{operator_name}_kernel.cpp`：基于模板中的 kernel.cpp 实现 AscendC Kernel 类 + `__aicore__` 入口，通过 `GM_ADDR` 接收 Tiling 指针并解析
   - 创建 `{operator_name}_host.cpp`：基于模板中的 host.cpp 实现参数校验、计算 TilingData、`aclrtMalloc`+`aclrtMemcpy` 传递到 Device、`<<<>>>` 直调 Kernel
   - 架构特定代码放在 `archXX/` 子目录
