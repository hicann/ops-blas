...
│       ├── blas-new-op-workflow/   # 主工作流 skill
...
**注意**：`op-samples-reference` 和 `asc-devkit-reference` 是**本地 skill**（在 `agent/skills/` 中维护），不在 `cannbot_references.json` 中。它们由 init.sh 步骤 5 自动创建软链接，与 `blas-log` 等本地 skill 处理方式相同。
