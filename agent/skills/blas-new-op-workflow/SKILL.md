...
10. **禁止提交临时文件** — `.agent/` 目录下的所有文件均为临时文件，已被 `.gitignore` 屏蔽。禁止在任何阶段执行 `git add .agent/` 或将该目录下的文件加入版本控制。若发现误加，必须立即 `git reset` 撤销。
11. **CP 否定分支统一打回上一步** — 用户在 CP 问卷中选择「需要修改」时，按以下路径打回：
