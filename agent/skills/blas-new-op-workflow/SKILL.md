---
name: blas-new-op-workflow
description: ops-blas 算子全流程开发技能，协调 agent 团队完成设计->开发->验收->上库的完整开发链路。触发：用户要求开发新算子或实现某 BLAS 接口时。
---

# 硬规则

以下规则在任何阶段都必须遵守，不可跳过、不可变通。

1. **禁止主 Agent 亲自动手** - 每个步骤启动后，主 Agent 只负责调用对应的 Subagent：编码→`developer`，测试→`tester`，设计→`architect`，检视→`reviewer`，文档→`writer`。**禁止**在主 Agent 层面做以下操作：读模板文件（assets/*.md）、读代码、搜索仓内文件、抓取/搜索网页资料、写代码、写文档。这些全部是 Subagent 的工作。主 Agent 唯一例外：读取 references/ 下的流程参考文件、更新 LOG.md、发送 AskUserQuestion
2. **调用只定义三要素** — 输入、输出、验收标准，禁止干涉 Subagent 的实现细节
3. **阶段不可跳过** — 设计→开发→验收→上库，每阶段需用户确认后推进
4. **门控不可绕过** — Subagent 报告未通过验证时，禁止进入下一阶段
5. **问卷不允许修改** — 基于已生成的 json 模板发送 `AskUserQuestion` 问卷时，不得进行任何修改，直接发送
6. **问卷结果落盘** — 每次问卷得到用户答复后，保存为 `{原问卷名}.ret.json`，仅保留用户选择的选项，删除未选选项
7. **后期默认批准 + 失败直接打回** — CP1.2 之后的 1.4 / 2.1 / 2.2 / 3.2：（a）评审/验收通过 → 直接进入下一步，不询问用户；（b）评审/验收不通过 → 直接打回修订，不询问用户，同一轮超过 3 次仍不通过再生成 CP 问卷询问用户。CP4.3 除外，必须询问用户
8. **日志即时更新** — 每次流程步骤推进后，主 Agent 必须立即更新 LOG.md（当前步骤、进度状态、开发记录），不得延迟或累积到后续步骤
9. **修复只走 developer** — 若检视/验收报告发现问题或需要修复的建议，必须通过 developer Subagent 修复，主 Agent 不得直接修改代码

---

# 启动指令

**加载本技能后，主 Agent 必须立即执行以下动作，不得做任何其他操作（包括读文件、搜索、询问用户）：**

1. 确定当前步骤（首次加载从 1.1 开始，中断恢复从 LOG.md 记录的步骤继续）
2. 查找下方流程表中该步骤对应的"参与角色"
3. 调用该 Subagent，传入 task-prompts.md 中定义的输入/输出/验收标准
4. Subagent 返回后，主 Agent 更新 LOG.md，然后按流程表推进到下一步

---

# 流程全景

## 角色

| 角色 | 职责 |
|------|------|
| 用户 | 需求提出、各确认点审批 |
| writer | 资料准备、文档与问卷整理、文档编写 |
| architect | 需求分析、方案设计、方案评审 |
| developer | 代码开发、编译联调、性能调优 |
| tester | 测试设计、用例开发、测试验收 |
| reviewer | 代码检视：规范、一致性、风险 |

## 流程

| 步骤 | 输入 | 参与角色 | 输出 | 说明 | 并行 |
|------|------|--------|------|------|------|
| **阶段1：设计** | | | | | |
| 1.1 资料准备 | 用户需求 | writer | 工作区目录、LOG.md、1.1-参考资料清单.md、CP1.1.json | 初始化目录 + 下载资料 + 整理问卷 | |
| ⛔ CP1.1 | CP1.1.json | 用户 | 基础信息对齐 | AskUserQuestion | |
| 1.2 需求分析 | CP1.1结论 + 1.1-参考资料清单.md | architect | 1.2-需求分析.md、CP1.2.json | 参数约束、可行性评估 + 整理为问卷 | |
| ⛔ CP1.2 | CP1.2.json | 用户 | 需求分析审批 | AskUserQuestion | |
| 1.3.A 开发方案设计 | 1.2-需求分析.md | architect | 1.3.A-开发方案设计.md | Tiling / Kernel / Host 设计 | ┐ |
| 1.4.A 开发方案评审 | 1.3.A-开发方案设计.md | architect | 1.4.A-开发方案评审.md | 不通过 → 打回 1.3.A，循环 ≤3 次 | │ |
| 1.3.B 测试方案设计 | 1.2-需求分析.md | tester | 1.3.B-测试方案设计.md | 用例表 + 验收标准 | │ |
| 1.4.B 测试方案评审 | 1.3.B-测试方案设计.md、1.2-需求分析.md | tester | 1.4.B-测试方案评审.md | 不通过 → 打回 1.3.B，循环 ≤3 次 | ┘ |
| ⚪ 1.4 | 1.4.A + 1.4.B | 主Agent | 裁定 | 双方通过→阶段2，任一方3次失败→CP问卷 | |
| **阶段2：开发** | | | | | |
| 2.0.1 环境准备 | 算子名 | developer | 2.0.1-开发环境.md、git 分支 | 环境检查、创建分支，不通过→AskUserQuestion | |
| **2.1 迭代一** | | | | 核心路径 | |
| 2.1.1.A 算子开发 | 1.3.A-开发方案设计.md | developer | 算子代码 | 核心逻辑 + 框架 | ┐ |
| 2.1.1.B 测试开发 | 1.3.B-测试方案设计.md | tester | L0 用例代码 | CSV + golden + GTest | ┘ |
| 2.1.2 汇合联调 | 算子代码 + 用例代码 | developer | 2.1.2-汇合联调报告.md | 编译 + L0 精度 | |
| 2.1.3 测试验收 | 2.1.2-汇合联调报告.md | tester | 2.1.3-测试验收报告.md | L0 通过率 100%，不通过→打回 | |
| ⚪ 2.1 | 2.1.3-测试验收报告.md | 主Agent | 裁定 | 通过→迭代二，不通过→打回 | |
| **2.2 迭代二** | | | | 全覆盖 + 边界 + 异常 | |
| 2.2.1.A 算子开发 | 1.3.A-开发方案设计.md | developer | 完整算子代码 | 补齐分支 + 异常拦截 | ┐ |
| 2.2.1.B 测试开发 | 1.3.B-测试方案设计.md | tester | L0+L1 用例代码 | 新增 CSV 行 + 测试分支 | ┘ |
| 2.2.2 汇合联调 | 算子代码 + 用例代码 | developer | 2.2.2-汇合联调报告.md | 编译 + 全量精度 | |
| 2.2.3 测试验收 | 2.2.2-汇合联调报告.md | tester | 2.2.3-测试验收报告.md | 全量通过率 100%，不通过→打回 | |
| ⚪ 2.2 | 2.2.3-测试验收报告.md | 主Agent | 裁定 | 通过→阶段3，不通过→打回 | |
| **阶段3：验收** | | | | | |
| 3.1 代码检视 | 全部代码 | reviewer | 3.1-代码检视报告.md | 规范、一致性、风险 | |
| 3.2 性能验收 | 1.2-需求分析.md、1.3.A-开发方案设计.md | developer | 3.2-性能报告.md | 性能采集、瓶颈分析 | |
| ⚪ 3.2 | 3.1 + 3.2 | 主Agent | 裁定 | 通过→阶段4，不通过→打回 | |
| **阶段4：上库** | | | | | |
| 4.1 编写文档 | 全部代码和设计文档 | writer | README.md | — | |
| 4.2 代码检视 | 全部代码和文档 | reviewer | 4.2-代码检视报告.md | 规范 + 冗余清理 | |
| 4.3 开发总结 | 全部交付物 | writer | CP4.3.json | 整理为问卷 | |
| ⛔ CP4.3 | CP4.3.json | 用户 | 上库审批 | AskUserQuestion，通过后 squash commit | |

**图例**：⛔ 必需确认  ⚪ 仅不通过时直接打回，3次仍失败后询问

---

# 参考资源

| 资源 | 路径 | 说明 |
|-----|------|------|
| Task 调用参数 | [references/task-prompts.md](references/task-prompts.md) | 各阶段 Subagent 的调用参数与验收标准 |
| 数据流说明 | [references/data-flow.md](references/data-flow.md) | 各阶段输入输出文件和数据流向 |
| 错误处理指南 | [references/error-handling.md](references/error-handling.md) | 常见错误类型、重试阈值与回退策略 |
| Issue 模板 | [assets/ISSUE_TEMPLATE.md](assets/ISSUE_TEMPLATE.md) | 问题记录模板 |
| 文档与问卷模板 | [assets/](assets/) | 所有产出物的模板文件 |
