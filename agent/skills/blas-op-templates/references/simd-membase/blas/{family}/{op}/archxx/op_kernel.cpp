...
public:
    ...
    __aicore__ inline void Init(/* GM_ADDR 各输入/输出, */ GM_ADDR tilingGm, TPipe* pipe);
...
__aicore__ inline void {{Op}}AIV::Init(/* GM_ADDR 各输入/输出, */ GM_ADDR tilingGm, TPipe* pipe)
{
    pipe_ = pipe;
...
__aicore__ inline void {{Op}}AIV::ParseTilingData(GM_ADDR tilingGm)
{
    __gm__ {{Op}}TilingData* tilingPtr = reinterpret_cast<__gm__ {{Op}}TilingData*>(tilingGm);
    // TEMPLATE: 逐字段拷贝到本地 tiling_
}
...
// - 参数列表 = 所有 GM 指针 + workSpace + tilingGm（按算子需求调整）
// - workSpace 是标准参数，即使不用也保留（传 nullptr）
// - KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY)
// - TPipe 在入口函数中创建
extern "C" __global__ __aicore__ void {{op}}_kernel(/* GM_ADDR ..., */ GM_ADDR workSpace, GM_ADDR tilingGm)
{
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY);
    TPipe pipe;
    {{Op}}AIV op;
    op.Init(/* ..., */ tilingGm, &pipe);
    op.Process();
}

// TEMPLATE: Kernel 启动器（host 侧调用）
// - 参数 = GM 指针 + workSpace + tilingGm + numBlocks + stream
void {{op}}_kernel_do(/* GM_ADDR ..., */ GM_ADDR workSpace, GM_ADDR tilingGm, uint32_t numBlocks, void* stream)
{
