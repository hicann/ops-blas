...
ops-blas 仓中的算子按 **编程模型** / **API 变体** 和 **目标架构** 维度分类：
...
除按编程模型分类外，ops-blas 仓存在一类**多数据类型扩展接口**（Ex 变体），其特征是 A/B/C（或 alpha/x）各路带独立 `aclDataType` + `aclblasComputeType_t computeType` 计算精度可配。Ex 变体**横跨多种编程模型**（矩阵类用 Cube，向量类用 Vector SIMD/SIMT），因此单列模板：

| API 变体 | 关键特征 | 代表算子 | 模板目录 |
|---------|---------|---------|---------|
| **Ex 变体** | 多 `aclDataType` + `computeType` + `DTypeCase` 派发 + alpha/beta 为 `void*` | gemm_ex / gemm_batched_ex / gemm_grouped_batched_ex / scalex | `references/ex-variant/` |

> **选型**：新算子若支持多 dtype + computeType，优先选 `ex-variant` 模板；否则按编程模型选 simd-membase/regbase/simt。详见 [references/ex-variant/README.md](references/ex-variant/README.md)。
...
- `ex-variant` 模板仅适用于 arch35（Ex 变体算子当前仅 arch35 实现），使用 `arch35/` 目录名
- `{op}` 为 snake_case 格式的算子名（如 `sswap`、`sgeqrf_batched`、`gemm_ex`）
...
| Ex 变体模板 | [references/ex-variant/](references/ex-variant/) | 以 gemm_ex/gemm_batched_ex/gemm_grouped_batched_ex/scalex 为原型抽取的 Ex 变体算子全套模板（仅 arch35） |
...
| **Ex 变体** | **gemm_ex** | **`blas/gemm/arch35/gemm_ex_*.cpp`**（单矩阵 Ex，Cube BlockMmad + alpha/beta 后处理） |
| **Ex 变体** | **gemm_batched_ex** | **`blas/gemm_batched_ex/arch35/`**（批量 Ex，指针数组 + workspace 布局） |
| **Ex 变体** | **gemm_grouped_batched_ex** | **`blas/gemm_grouped_batched_ex/arch35/`**（分组 Ex，GM 平铺 tiling + cube/epilogue 两段） |
| **Ex 变体** | **scalex** | **`blas/scalex/arch35/`**（向量 Ex，SIMD/SIMT 双路径 + 混合精度中转） |
