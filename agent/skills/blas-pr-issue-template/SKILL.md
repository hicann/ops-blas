...
```
问题：请提供你的 GitCode access token（用于 API 认证）。
      如果还没有，可以在 GitCode 个人设置 -> Access Tokens 中生成。
选项：
- 我来提供 token（用户在自定义输入中粘贴）
```

用户回复 token 后，将其保存为变量 `TOKEN`，后续所有 API 调用均使用此 token。
...
如果 commit 数量大于 1，使用 `question` 工具询问用户：

```
问题：当前分支有 N 个 commit，是否需要合并为单个 commit？
选项：
- 合并为单个 commit（推荐）
- 保留所有 commit
```
...
如果用户选择触发，使用 `scripts/comment_pr.sh` 添加评论：
...
| `comment_pr.sh` | 在 PR 评论区添加评论 | `scripts/comment_pr.sh` |
