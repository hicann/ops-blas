...
description: 生成 PR 或 Issue 文件，并可通过 GitCode API 直接提交到仓库。根据上下文推断内容，发送问卷让用户逐项确认，最终在当前工作目录生成 md 文件。触发：用户要求创建 PR、提交 Issue、填写变更说明或问题反馈、向已创建的 PR 推送代码时。关键词：提 PR、创建 PR、提交 Issue、推送代码到 PR、amend、force push。
---
...
**示例：**
```bash
bash scripts/update_pr.sh "abc123" "cann/ops-blas" "132" ".agent/pr-body-update.md"
```
