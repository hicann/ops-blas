...
Sasum(向量元素绝对值之和)算子实现了计算向量元素绝对值的和，是BLAS基础线性代数库中的核心算子之一。

该算子计算L1范数（曼哈顿范数），常用于向量稀疏度度量和误差估计。
...
- 算子规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">sasum_kernel</td></tr>
  </table>

- 算子实现： 

    将输入数据从x的GM地址分块搬运到UB，并行计算各核的局部绝对值之和，最后归约得到全局总和。
...
- 样例执行
  ```bash
  bash build.sh --ops=sasum --run # --ops=<算子名> --run可选参数，执行测试样例
  ```
  执行结果如下，说明精度对比成功。
  ```bash
