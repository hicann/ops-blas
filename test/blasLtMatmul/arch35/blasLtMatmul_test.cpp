...
aclError aclblasLtMatmulTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    CHECK_ACLBLAS(aclblasLtMatmulPreferenceSetAttribute(
        preference, ACLBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES, &max_workspaceSize, sizeof(max_workspaceSize)));

    // 用于接收推荐算法
    int returnedAlgoCount = 0;
    const int request_solutions = 1;
    aclblasLtMatmulHeuristicResult_t heuristicResult[request_solutions];
    CHECK_ACLBLAS(aclblasLtMatmulAlgoGetHeuristic(
        ltHandle, operationDesc, Adesc, Bdesc, Ddesc, Ddesc, preference, request_solutions, heuristicResult,
        &returnedAlgoCount));
    ...
    CHECK_ACLBLAS(aclblasLtMatmul(
        ltHandle, operationDesc, &alpha, d_A, Adesc, d_B, Bdesc, &beta, d_D, Ddesc, d_D, Ddesc, &algo, d_workspace,
