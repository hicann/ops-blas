...
static void GoldenUpper(
    const std::vector<float>& x, float alpha, const std::vector<float>& A_orig, std::vector<float>& A_golden, int64_t n,
    int64_t lda)
{
    A_golden = A_orig;
...
static uint32_t VerifyTriRegion(
    const float* out, const float* gold, int64_t n, int64_t lda, aclblasFillMode uplo, const char* nm)
{
    ...
    if (pass)
        ...
        LOG_PRINT(
            "  [%s] Triangular FAILED (MERE=%.6e, MARE=%.6e, limit=%.6e/%.6e)\n", nm, mere, mare, MERE_THRESHOLD,
            mareThreshold);
    return pass ? 0 : 1;
}
...
static uint32_t VerifyUnchangedRegion(
    const float* out, const float* orig, int64_t n, int64_t lda, aclblasFillMode uplo, const char* nm)
{
    uint32_t err = 0;
    for (int64_t row = 0; row < n; ++row) {
        ...
        for (int64_t col = c0; col < c1; ++col) {
            if (out[row * lda + col] != orig[row * lda + col])
                ++err;
        }
    }
    if (err == 0)
        LOG_PRINT("  [%s] Unchanged PASSED\n", nm);
    else
        LOG_PRINT("  [%s] Unchanged FAILED: %u errors\n", nm, err);
    return err;
}

static uint32_t VerifyFull(const float* out, const float* gold, int64_t n, int64_t lda, const char* nm)
{
    double sumRelErr = 0.0;
    float maxRelErr = 0.0f;
    int64_t total = n * lda;
    ...
    if (pass)
        ...
        LOG_PRINT(
            "  [%s] Full FAILED (MERE=%.6e, MARE=%.6e, limit=%.6e/%.6e)\n", nm, mere, mare, MERE_THRESHOLD,
            mareThreshold);
    return pass ? 0 : 1;
}

static aclblasStatus_t Run(
    aclblasHandle h, aclrtStream s, aclblasFillMode uplo, int n, float alpha, const std::vector<float>& x, int incx,
    std::vector<float>& A, int lda)
{
    ...
    if (aclrtMalloc((void**)&dx, xb, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS)
        return ACLBLAS_STATUS_ALLOC_FAILED;
    if (aclrtMalloc((void**)&da, ab, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
        aclrtFree(dx);
        return ACLBLAS_STATUS_ALLOC_FAILED;
    }
...
static uint32_t L002(aclblasHandle h, aclrtStream s)
{
    ...
    for (int64_t i = 0; i < n * lda; ++i)
        A[i] = (i % 100) - 50;
    std::vector<float> gold;
    GoldenLower(x, alpha, A, gold, n, lda);
    auto out = A;
...
static uint32_t L101(aclblasHandle h, aclrtStream s)
{
    ...
    for (int64_t i = 0; i < n; ++i)
        xd[i] = x[i * incx];
    std::vector<float> gold;
    GoldenUpper(xd, alpha, A, gold, n, lda);
    auto out = A;
    Run(h, s, ACLBLAS_UPPER, n, alpha, x, incx, out, lda);
...
static uint32_t L102(aclblasHandle h, aclrtStream s)
{
    ...
    for (int64_t i = 0; i < n; ++i)
        xd[i] = x[i * incx];
    std::vector<float> gold;
    GoldenLower(xd, alpha, A, gold, n, lda);
    auto out = A;
    Run(h, s, ACLBLAS_LOWER, n, alpha, x, incx, out, lda);
...
{
    int n = 64, incx = -1, absIncx = 1, lda = n;
    float alpha = 2.0f;
    ...
    for (int64_t i = 0; i < n; ++i)
        xMem[i * absIncx] = xLogical[n - 1 - i];
    std::vector<float> A(n * lda);
    for (int64_t i = 0; i < n * lda; ++i)
        A[i] = (i % 100) - 50;
    std::vector<float> gold;
    GoldenUpper(xLogical, alpha, A, gold, n, lda);
    auto out = A;
    Run(h, s, ACLBLAS_UPPER, n, alpha, xMem, incx, out, lda);
...
{
    int n = 64, incx = -2, absIncx = 2, lda = n;
    float alpha = 1.5f;
    std::vector<float> xLogical(n);
    for (int64_t i = 0; i < n; ++i)
        xLogical[i] = (i * 3) % 11;
    ...
    for (int64_t i = 0; i < n * lda; ++i)
        A[i] = i % 13;
    std::vector<float> gold;
    GoldenLower(xLogical, alpha, A, gold, n, lda);
    auto out = A;
    Run(h, s, ACLBLAS_LOWER, n, alpha, xMem, incx, out, lda);
...
static uint32_t L206(aclrtStream s)
{
    ...
    GoldenUpper(x, alpha, A, gold, n, lda);
    auto out = A;
    Run(nullptr, s, ACLBLAS_UPPER, n, alpha, x, 1, out, lda);
...
int main()
{
    ...
    LOG_PRINT("========================================\n");
    if (te == 0)
        LOG_PRINT("  RESULT: ALL TESTS PASSED\n");
    else
        LOG_PRINT("  RESULT: %u FAILED\n", te);
