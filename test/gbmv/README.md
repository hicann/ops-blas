...
├── gbmv_param.h              // 参数结构体（继承 BlasTestParamBase）
├── gbmv_golden.h             // CPU golden（签名与 BLAS API 一致）
└── arch35/
    ├── gbmv_npu_wrapper.h    // NPU wrapper（封装 aclrtMalloc/H2D/kernel/D2H/free）
    ├── gbmv_test.cpp         // 精度测试（GTest 入口）
    └── gbmv_test.csv         // 精度测试用例表
```
