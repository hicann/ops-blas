...
class ConfigLoader {
    ...
    static std::vector<TestCaseConfig> loadCsvTestCases(const std::string& csvPath) {
        ...
        while (std::getline(ifs, line)) {
            ...
            for (size_t i = 0; i < headers.size(); i++) {
                ...
                else if (key == "sparam0") tc.sparam0 = parseOptFloat(val);
                else if (key == "sparam1") tc.sparam1 = parseOptFloat(val);
                else if (key == "sparam2") tc.sparam2 = parseOptFloat(val);
                else if (key == "sparam3") tc.sparam3 = parseOptFloat(val);
                else if (key == "sparam4") tc.sparam4 = parseOptFloat(val);
                else if (key == "seed") {
    ...
    static std::pair<std::vector<TestCaseConfig>, OpConfig> loadLegacyJson(const std::string& jsonPath) {
        ...
        for (size_t pos : objPositions) {
            ...
            tc.sparam0 = extractFloat("sparam0", pos);
            tc.sparam1 = extractFloat("sparam1", pos);
            tc.sparam2 = extractFloat("sparam2", pos);
            tc.sparam3 = extractFloat("sparam3", pos);
            tc.sparam4 = extractFloat("sparam4", pos);
