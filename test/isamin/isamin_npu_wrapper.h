...
inline std::unique_ptr<DeviceBuffer> tryAllocAndCopyIsamin(const void* hostPtr, size_t bytes)
{
    if (hostPtr == nullptr)
        return nullptr;
...
inline aclblasStatus_t aclblasIsamin_npu(aclblasHandle_t handle, int n, const float* x, int incx, int* result)
{
    if (handle == nullptr) {
        return ACLBLAS_STATUS_NOT_INITIALIZED;
    }
    if (n <= 0 || incx <= 0) {
        return aclblasIsamin(handle, n, x, incx, result);
    }
    ...
    auto dX = tryAllocAndCopyIsamin(x, dataBytes);
    auto dResult = std::make_unique<DeviceBuffer>(resultBytes);

    aclblasStatus_t ret = aclblasIsamin(
