...
INSTANTIATE_TEST_SUITE_P(
    Sasum, SasumArch35Test,
    ::testing::ValuesIn(GetCasesFromCsv<SasumParam>(ReplaceFileExtension2Csv(__FILE__))),
    PrintCaseInfoString<SasumParam>);

TEST_P(SasumArch35Test, CsvDriven) {
    const auto& p = GetParam();
    std::cout << "\n=== RUN  [" << p.caseName << "] n=" << p.n << " incx=" << p.incx
              << " x=" << static_cast<int>(p.x) << " desc=" << p.description << " ===" << std::endl;

    const int64_t xLen = (p.n > 0) ? static_cast<int64_t>(1 + (p.n - 1) * p.incx) : 0;
    std::vector<float> xHost = makeBlasArray(xLen, p.x, p.description);
