...
__aicore__ __inline__ void matrix_gm2ubuf_cce(__ubuf__ float *dst, __gm__ float *src, int64_t m_actual,
...
__aicore__ __inline__ void matrix_ubuf2gm_cce(__gm__ float *dst, __ubuf__ float *src,
                                         ...
                                         float alpha, uint32_t lessFlag, __gm__ float *__restrict__ AIV_AIC_workspace)
{
    ...
    for (int64_t loop_idx = 0; loop_idx < loop; loop_idx++) {
        ...
        if (get_subblockid() == 0) {
            matrix_gm2ubuf_cce(ubufC, gm_C + offset_c, M0 / 2 < m_actual ? M0 / 2 : m_actual, n_actual, N, N0);
        } else if (get_subblockid() == 1 && M0 / 2 < m_actual) {
            matrix_gm2ubuf_cce(ubufC, gm_C + offset_c + get_subblockid() * N * M0 / 2, m_actual - M0 / 2,
                                       n_actual, N, N0);
        }
        ...
        if (get_subblockid() == 0) {
            matrix_ubuf2gm_cce(gm_C + offset_c, ubufC, M0 / 2 < m_actual ? M0 / 2 : m_actual, n_actual, N0, N);
        } else if (get_subblockid() == 1 && M0 / 2 < m_actual) {
            matrix_ubuf2gm_cce(gm_C + offset_c + get_subblockid() * N * M0 / 2, ubufC, m_actual - M0 / 2,
                                       n_actual, N0, N);
        }
