...
测试代码位于 `test/gbmv/sgbmv/`：

```
test/gbmv/sgbmv/
├── CMakeLists.txt              // 编译工程文件
├── sgbmv_param.h               // 参数结构体（继承 BlasTestParamBase）
├── sgbmv_golden.h              // CPU golden（基于 CBLAS cblas_sgbmv）
└── arch35/
...
- 算子规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="6" align="center">sgbmv_kernel</td></tr>
  </table>

- 算子实现： 

    Kernel 侧采用双路径策略：incx==1 时将 x 向量加载到 UB 缓存减少 GM 访问；否则回退到 GM 直接访问模式。多核并行通过循环行分布实现负载均衡。TilingData 通过值传递下发至 Kernel，无需设备侧内存分配。Host 侧不执行流同步，由调用方自行管理。
