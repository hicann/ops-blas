    ...
    const float* beta, const float* A, const float* x, const float* y)
{
    ...
    CHECK_RET(kl >= 0, return ACLBLAS_STATUS_INVALID_VALUE);
    CHECK_RET(ku >= 0, return ACLBLAS_STATUS_INVALID_VALUE);
    CHECK_RET(lda >= kl + ku + 1, return ACLBLAS_STATUS_INVALID_VALUE);
    ...
    int incx, int incy)
{
    ...
    tilingData.numThreads =
        std::min(CeilAlign<uint32_t>(CeilDiv<uint32_t>(outDim, useNumBlocks), SIMT_MIN_THREAD_NUM), SIMT_MAX_THREAD_NUM);
    ...
    int lda, const float* x, int incx, const float* beta, float* y, int incy)
{
    ...
    CHECK_RET(m >= 0 && n >= 0, return ACLBLAS_STATUS_INVALID_VALUE);
    if (m == 0 || n == 0) {
        return ACLBLAS_STATUS_SUCCESS;
    }
    ...
    SgbmvTilingData tiling =
        CalSgbmvTilingData(useNumBlocks, m, n, kl, ku, lda, trans, *alpha, *beta, incx, incy);
    ...
    aclRet = aclrtMemcpy(
        tilingDevice, sizeof(SgbmvTilingData), &tiling, sizeof(SgbmvTilingData), ACL_MEMCPY_HOST_TO_DEVICE);
