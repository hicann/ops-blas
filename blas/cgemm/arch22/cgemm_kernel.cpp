...
__aicore__ __inline__ void Smatmul(aclblasOperation_t transA, aclblasOperation_t transB, int64_t M, int64_t N,
                                          ...
                                          AscendC::GlobalTensor<float> AIV_AIC_workspace)
{
    ...
    for (int64_t loop_idx = 0; loop_idx < loop; loop_idx++) {
        ...
        for (int64_t k_idx = 0; k_idx < k_loop; k_idx++) {
            ...
            if (transA != ACLBLAS_OP_N) {
                matrix_gm2cbuf_ND2nZ(L1_buf_a, gm_A[offset_a], K0, M0, k_actual, m_actual, K0);
            } else {
                matrix_gm2cbuf_ND2nN(L1_buf_a, gm_A[offset_a], M0, K0, m_actual, k_actual, M0);
            }
            SET_FLAG(MTE2, MTE1, K_LOOP_EVENT_ID);

            WAIT_FLAG(MTE1, MTE2, K_LOOP_EVENT_ID + 2);
            if (transB != ACLBLAS_OP_N) {
                matrix_gm2cbuf_ND2nN(L1_buf_b, gm_B[offset_b], N0, K0, n_actual, k_actual, N0);
            } else {
                matrix_gm2cbuf_ND2nZ(L1_buf_b, gm_B[offset_b], K0, N0, k_actual, n_actual, K0);
            }
                                        ...
                                        AscendC::GlobalTensor<float> src, int64_t M, int64_t N, int64_t dst_pad, int64_t src_pad)
{
    ...
    for (int64_t pi = 0; pi < repeats_per_core; pi++) {
        ...
        matrix_gm2ubuf(complex_buf, src[(c_id * n_per_repeat * src_pad + r_id * NUM_PER_REPEAT) * 2],
...
__aicore__ __inline__ void CgemmPre(aclblasOperation_t transA, aclblasOperation_t transB, int64_t M, int64_t N,
                                           int64_t K, aclblasComplex alpha, AscendC::GlobalTensor<float> d_A, int64_t lda,
                                           AscendC::GlobalTensor<float> d_B, int64_t ldb, aclblasComplex beta,
...
{
    if (transA == ACLBLAS_OP_N) splitMatrix(d_A_r, d_A_i, d_A, M, K, lda_pad, lda);
    else splitMatrix(d_A_r, d_A_i, d_A, K, M, lda_pad, lda);
    if (transB == ACLBLAS_OP_N) splitMatrix(d_B_r, d_B_i, d_B, K, N, ldb_pad, ldb);
...
__aicore__ __inline__ void mulSComplex(AscendC::LocalTensor<float> src_r, AscendC::LocalTensor<float> src_i,
                                        ...
                                        int64_t n_blocks, aclblasComplex beta)
{
...
__aicore__ __inline__ void CgemmFinal(aclblasOperation_t transA, aclblasOperation_t transB, int64_t M, int64_t N,
                                             int64_t K, aclblasComplex alpha, AscendC::GlobalTensor<float> d_A, int64_t lda,
                                             AscendC::GlobalTensor<float> d_B, int64_t ldb, aclblasComplex beta,
                                             AscendC::GlobalTensor<float> d_C, int64_t ldc, int64_t lda_pad, int64_t ldb_pad,
                                             AscendC::GlobalTensor<float> d_A_r, AscendC::GlobalTensor<float> d_A_i,
                                             ...
                                             AscendC::GlobalTensor<float> d_C_ir, AscendC::GlobalTensor<float> d_C_ii)
{
    ...
    for (int64_t pi = 0; pi < repeats_per_core; pi++) {
        ...
        matrix_gm2ubuf(complex_buf, d_C[(c_id * n_per_repeat * ldc + r_id * NUM_PER_REPEAT) * 2],
        ...
        matrix_gm2ubuf(complex_buf_real, d_C_rr[(c_id * n_per_repeat * ldc + r_id * NUM_PER_REPEAT)], m_len, n_len, ldc, m_round);
        matrix_gm2ubuf(complex_buf_imag, d_C_ri[(c_id * n_per_repeat * ldc + r_id * NUM_PER_REPEAT)], m_len, n_len, ldc, m_round);
        matrix_gm2ubuf(imag_tbuf, d_C_ir[(c_id * n_per_repeat * ldc + r_id * NUM_PER_REPEAT)], m_len, n_len, ldc, m_round);
        matrix_gm2ubuf(real_tbuf, d_C_ii[(c_id * n_per_repeat * ldc + r_id * NUM_PER_REPEAT)], m_len, n_len, ldc, m_round);
        SET_FLAG(MTE2, V, EVENT_ID1);
        WAIT_FLAG(MTE2, V, EVENT_ID1);
        if ((transA == ACLBLAS_OP_C) != (transB == ACLBLAS_OP_C))
        ...
        matrix_ubuf2gm(d_C[(c_id * n_per_repeat * ldc + r_id * NUM_PER_REPEAT) * 2], complex_buf,
                                            ...
                                            __gm__ uint8_t *__restrict__ tilingGm)
{
    ...
    aclblasOperation_t _transA = transA == 0 ? ACLBLAS_OP_N : (transA == 1 ? ACLBLAS_OP_T : ACLBLAS_OP_C);
    aclblasOperation_t g_transB = transB == 0 ? ACLBLAS_OP_N : (transB == 1 ? ACLBLAS_OP_T : ACLBLAS_OP_C);

    WaitFlagDev(0);
    Smatmul(_transA, g_transB, M, N, K, d_A_r_tensor, lda_pad, d_B_r_tensor, ldb_pad, d_C_rr_tensor, ldc, 1, 128, 128, 128, 0, workspace_tensor);
    Smatmul(_transA, g_transB, M, N, K, d_A_r_tensor, lda_pad, d_B_i_tensor, ldb_pad, d_C_ri_tensor, ldc, 1, 128, 128, 128, 0, workspace_tensor);
    Smatmul(_transA, g_transB, M, N, K, d_A_i_tensor, lda_pad, d_B_r_tensor, ldb_pad, d_C_ir_tensor, ldc, 1, 128, 128, 128, 0, workspace_tensor);
    Smatmul(_transA, g_transB, M, N, K, d_A_i_tensor, lda_pad, d_B_i_tensor, ldb_pad, d_C_ii_tensor, ldc, 1, 128, 128, 128, 0, workspace_tensor);
    ...
    PIPE_BARRIER(ALL);
    aclblasComplex alpha{alpha_r, alpha_i};
    aclblasComplex beta{beta_r, beta_i};
    ...
    CgemmPre(_transA, g_transB, M, N, K, alpha, d_A_tensor, lda, d_B_tensor, ldb, beta, d_C_tensor, ldc,
    ...
    WaitFlagDev(1);
    CgemmFinal(_transA, g_transB, M, N, K, alpha, d_A_tensor, lda, d_B_tensor, ldb, beta, d_C_tensor, ldc,
                      lda_pad, ldb_pad, d_A_r_tensor, d_A_i_tensor, d_B_r_tensor, d_B_i_tensor,
                      d_C_rr_tensor, d_C_ri_tensor, d_C_ir_tensor, d_C_ii_tensor);
    PIPE_BARRIER(ALL);
#endif
