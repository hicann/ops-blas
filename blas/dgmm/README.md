...
```
LEFT 模式：  B = diag(x) * A，  B[i,j] = x[i] * A[i,j]    （x 长度为 m，按行缩放）
RIGHT 模式： B = A * diag(x)，  B[i,j] = A[i,j] * x[j]    （x 长度为 n，按列缩放）
```
...
```cpp
aclblasStatus_t aclblasSdgmm(aclblasHandle_t handle, aclblasSideMode_t mode, int m, int n, const float *x, int incx, const float *A, int lda, float *B, int ldb)
```
...
| m | 输入 | int | 矩阵 A/B 的行数，m >= 0，Host 内存 |
| n | 输入 | int | 矩阵 A/B 的列数，n >= 0，Host 内存 |
...
| B | 输出 | float*（FP32） | 输出矩阵，列主序存储，维度 m×n，Device 内存 |
| ldb | 输入 | int | 矩阵 B 的主维（leading dimension），ldb >= max(1, m)，Host 内存 |
...
- 当 m > 0 且 n > 0 时，x、A、B 不能为 nullptr，否则返回 ACLBLAS_STATUS_INVALID_VALUE
...
以下示例演示 LEFT 模式（B = diag(x) * A），取 m = 3, n = 3, x = [2, 3, 4]：
...
int aclblasSdgmmTest(AclContext& ctx)
{
    ...
    size_t bBytes = static_cast<size_t>(ldb) * static_cast<size_t>(n) * sizeof(float);
    ...
    aclRet = aclrtMalloc(&bRawMem, bBytes, ACL_MEM_MALLOC_HUGE_FIRST);
    ...
    blasRet = aclblasSdgmm(
        ...
        static_cast<float*>(bDevicePtr.get()), ldb);
    ...
    std::vector<float> resultData(ldb * n, 0.0f);
    aclRet = aclrtMemcpy(resultData.data(), bBytes, bDevicePtr.get(), bBytes, ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(aclRet == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", aclRet); return aclRet);
    for (int col = 0; col < n; col++) {
        for (int row = 0; row < m; row++) {
            LOG_PRINT("B[%d,%d] = %f\n", row, col, resultData[col * ldb + row]);
        }
    }
