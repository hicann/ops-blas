...
## 一、贡献新算子
  ...
  如果您有全新的BLAS算子希望基于NPU设计与实现，欢迎在Issue中提出您的想法与设计方案。完整的贡献过程如下：
...
- 文档交付件：算子README文档为必选，其余文档可视情况提供。
...
如果您希望贡献项目标准算子，其交付件和开发过程比生态算子复杂，包括多架构支持、Tiling实现、GTest测试框架等，具体贡献指导参考[附录](#附录)。
...
├── README.md                                          # 算子说明文档
└── ${arch_dir}/                                       # 面向特定架构的实现（如 arch35、arch22）
    ├── ${operator}_host.cpp                           # Host侧：参数校验、Tiling计算、任务下发
    ├── ${operator}_kernel.cpp                         # Device侧：Ascend C / SIMT 核函数实现
    └── ${operator}_tiling_data.h                      # 可选，Tiling数据结构定义

test/${family}/${operator}/                            # 算子测试目录
├── CMakeLists.txt                                     # 测试编译配置
├── ${operator}_param.h                                # 测试参数结构体
├── ${operator}_golden.h                               # CPU golden参考实现
└── ${arch_dir}/                                       # 架构专用测试源
    ├── ${operator}_npu_wrapper.h                      # NPU调用封装
    ├── ${operator}_test.cpp                           # GTest测试主文件
    └── ${operator}_test.csv                           # CSV驱动的测试用例数据
```

> **说明**：
> - `${family}` 为算子族名（去除精度前缀），如 `trsm`、`dot`、`gemv`、`copy`、`gbmv`。
> - `${operator}` 为具体算子名（含精度前缀），如 `strsm`、`htrsm`、`sdot`、`scopy`。
> - `${arch_dir}` 为架构子目录，如 `arch22`（Ascend 910B）、`arch35`（Ascend 950），由编译时 `--soc` 参数决定。
...
- 代码交付件：需提供多架构支持（如 arch22、arch35）、Tiling实现、GTest测试框架（golden、param、npu_wrapper、CSV驱动测试）。
- 文档交付件：算子README文档需包含接口说明、参数约束、算法流程、执行路径、编译测试命令、调用示例等内容。
